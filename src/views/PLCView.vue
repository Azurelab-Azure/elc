<template>
  <div class="page">
    <header class="page-header">
      <h1 class="page-title">PLC</h1>
      <p class="page-desc">基于 rules/plc 规则驱动</p>
    </header>
    ...
    <div v-else class="layout">
      <section class="panel">
        ...
        <div v-if="rules.length === 0" class="empty">
          <p class="empty-title">暂无 PLC 规则</p>
          <p class="empty-desc">请将规则 JSON 放到 rules/plc/ 目录</p>
        </div>
      ...
      <section class="panel">
        ...
        <div v-if="plcCircuits.length === 0" class="empty">
          <p class="empty-title">暂无 PLC 电路</p>
          <p class="empty-desc">请将 HTML 文件放到 beta/circuits/plc/ 目录</p>
        </div>

        <div v-else class="circuit-list">
          <router-link
            v-for="circuit in plcCircuits"
            :key="circuit.id"
...
const { rules, loading, loadRules } = usePLC();
const { circuits, load } = useCatalog();

const plcCircuits = computed(() =>
  circuits.value.filter(c => c.category === 'plc')
);
